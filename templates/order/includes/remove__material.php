...
if($_SERVER["REQUEST_METHOD"] == "GET" AND isset($_GET["removeMaterialFromOrder"]) ) {
  ...
  $order_material = $entityManager->find("\App\Entity\OrderMaterial", $order_material_id);

  // First remove properties from table v6_orders_materials_properties.
  if ($order_material_properties = $entityManager->getRepository('\App\Entity\OrderMaterialProperty')->getOrderMaterialProperties($order_material_id)) {
