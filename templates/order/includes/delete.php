...
if($_SERVER["REQUEST_METHOD"] == "GET" AND isset($_GET["deleteOrder"]) ) {
  ...
  if ($order = $entityManager->find("\App\Entity\Order", $order_id)) {
    
    // Check if exist Materials in Order.
    if ($order_materials = $entityManager->getRepository('\App\Entity\OrderMaterial')->getOrderMaterials($order_id)) {

      // Loop trough all materials
      foreach ($order_materials as $order_material) {
        
        // Check if exist Properties in Order Material
        if ($order_material_properties = $entityManager->getRepository('\App\Entity\OrderMaterialProperty')->getOrderMaterialProperties($order_material->getId())) {
