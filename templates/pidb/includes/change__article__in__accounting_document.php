...
if($_SERVER["REQUEST_METHOD"] == "POST" AND isset($_GET["editArticleDataInAccountingDocument"]) ) {
  ...
  $pidb_article = $entityManager->find('\App\Entity\AccountingDocumentArticle', $pidb_article_id);

  $old_article = $entityManager->find('\App\Entity\Article', $pidb_article->getArticle()->getId());
  $old_article_id = $old_article->getId(); 
  
  $new_article_id = htmlspecialchars($_POST["article_id"]);
  $new_article = $entityManager->find('\App\Entity\Article', $new_article_id);
  ...
  if ($old_article_id == $new_article_id){
    ...
    if ($accounting_document__article__properties = $entityManager->getRepository('\App\Entity\AccountingDocumentArticleProperty')->findBy(array('accounting_document_article' => $pidb_article_id), array())) {
    ...
    $article_properties = $entityManager->getRepository('\App\Entity\ArticleProperty')->getArticleProperties($new_article->getId());
