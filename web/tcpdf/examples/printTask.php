...
$task_data = $task->getTask($task_id);
$client_data = $client->getClient($task_data['client_id']);

    $contacts = $contact->getContactsById($task_data['client_id']);
...
<table border="0">
  <tr><td width="150px"><h3>RADNI NALOG </h3></td><td>'.date('d M Y', strtotime($task_data['date'])).'</td></tr>
</table>
...
<table border="1">
  <tr><td>'.$task_data['title'].'</td></tr>
  <tr><td>'.nl2br($task_data['description']).'</td></tr>
</table>
';
