...
if(isset($_GET["exportProformaToDispatch"]) ) {
    ...
    // get proforma data
    $result_pidb = $connection->query("SELECT * FROM pidb WHERE id = '$pidb_id' ") or die(mysqli_error($connection));
    ...
    $result_pidb_avans = $connection->query("SELECT * FROM payment WHERE pidb_id = '$pidb_id'") or die(mysqli_error($connection));
    if($result_pidb_avans->num_rows) {
        ...
        $connection->query("INSERT INTO payment (date, pidb_id, payment_type_id, amount) VALUES ('$date', '$pidb_id_last', '$payment_type_id', '$amount' )") or die(mysqli_error($connection)); 
    }
