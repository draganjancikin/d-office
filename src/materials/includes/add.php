...
if($_SERVER["REQUEST_METHOD"] == "POST" AND isset($_GET["new"]) ) {
    ...
    $connect_db->query("INSERT INTO material (date, name, unit_id, weight, price,  note ) VALUES ('$date', '$name', '$unit_id', '$weight', '$price', '$note' )") or die(mysqli_error($connect_db));

    $material_id = $connect_db->insert_id;
...
if($_SERVER["REQUEST_METHOD"] == "POST" AND isset($_GET["newSupplier"]) ) {  
  ...
  $connect_db->query("INSERT INTO material_suppliers (material_id, client_id, code, price, date ) "
                   . "VALUES ('$material_id', '$client_id', '$code', '$price', '$date' )") or die(mysqli_error($connect_db));
...
if($_SERVER["REQUEST_METHOD"] == "POST" AND isset($_GET["newProperty"]) ) {
    ...
    $connect_db->query("INSERT INTO material_property (material_id, property_id, min, max) "
                     . "VALUES ('$material_id', '$property_item_id', '$min', '$max')") or die(mysqli_error($connect_db));
