...
if($_SERVER["REQUEST_METHOD"] == "GET" AND isset($_GET['delNote']) ) {
    ...
    $connection->query("DELETE FROM project_note WHERE id='$note_id' ") or die(mysqli_error($connection));
...
if($_SERVER["REQUEST_METHOD"] == "GET" AND isset($_GET['delTask']) ) {
  ...
  $result_project_task_notes = $connection->query("SELECT * FROM project_task_note WHERE project_task_id='$task_id'") or die(mysqli_error($connection));
  ...
  $connection->query("DELETE FROM project_task WHERE id='$task_id' ") or die(mysqli_error($connection));
...
if($_SERVER["REQUEST_METHOD"] == "GET" AND isset($_GET['delTaskNote']) ) {
    ...
    $connection->query("DELETE FROM project_task_note WHERE id='$task_note_id' ") or die(mysqli_error($connection));
