...
class Project {
    ...
    // zatim ide definisanje metoda (funkcija)==================================
    ...
    public function getCityName($client_id){
        ...
        $result_client = $this->connection->query("SELECT client.city_id, city.city_name "
                                         . "FROM client "
                                         . "JOIN (city)"
    ...
    public function getProject ($project_id){
        ...
        $result = $this->connection->query("SELECT project.id, project.pr_id, project.date, project.client_id, project.title, project.priority_id, project.status, project.note, project.created_at_user_id, client.name "
                                         . "FROM project "
                                         . "JOIN (client) "
    ...
    public function projectTracking ($status){
        ...
        while($row_project = mysqli_fetch_array($result)):
            ...
            $project_pr_id = $row_project['pr_id'];
    ...
    public function getCitysByActiveProject (){
        ...
        $result = $this->connection->query("SELECT DISTINCT city.id, city.city_name "
                                          . "FROM city "
                                          . "JOIN (client, project) "
                                          . "ON (client.city_id=city.id AND project.client_id=client.id) "
                                          . "WHERE project.status='1'"
                                          . "ORDER BY city.city_name") or die(mysqli_error($this->connection));
        while($row = mysqli_fetch_array($result)){
    ...
    public function projectAdvancedSearch ($client, $project_title, $city){
        ...
        $result =  $this->connection->query("SELECT project.id, project.pr_id, client.name, city.city_name, street.street_name, client.home_number, project.title, project.status, project.client_id "
                                          . "FROM client "
    ...
    public function delNoteFromProjectTask ($project_task_note_id){
        ...
        $this->connection->query("DELETE FROM project_task_note WHERE id='$project_task_note_id' ") or die(mysqli_error($this->connection));
