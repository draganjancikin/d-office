    ...
    <div class="table-responsive">
      <table class="table">
        ...
        <tbody>
          ...
          $total_kap = 0;
          $cutting_sheet_articles = $entityManager->getRepository('\App\Entity\CuttingSheet')
            ->getArticlesOnCuttingSheet($cutting_id);
          foreach ($cutting_sheet_articles as $cutting_sheet_article):
            $count ++;
            ?>
            <form action="<?php echo '/cutting/' . $cutting_id . '/article/' . $cutting_sheet_article->getId() .
              '/edit'?>"
                  method="post">
              ...
              <tr>
                ...
                <td class="px-1">
                  <select name="fence_model_id" required >
                    <option value="<?php echo $cutting_sheet_article->getFenceModel()->getId() ?>">
                      <?php echo $cutting_sheet_article->getFenceModel()->getName() ?>
                    </option>
                    <?php
                ...
                <td class="px-1">
                  ...
                  <a onClick="javascript: return confirm('Da li ste sigurni da želite da uklonite artikal iz krojne liste?')"
                     href="<?php echo '/cutting/' . $cutting_id . '/article/' . $cutting_sheet_article->getId() . '/delete' ?>">
          ...
          <tr>
            ...
            <td colspan="5">
              ...
              if ($cutting_sheet_articles):
                ?>
                <a href="<?php echo $_SERVER['PHP_SELF']. '?exportToAccountingDocument&id=' . $cutting_id . '&total_picket_lenght=' .$total_picket_lenght. '&total_kap=' .$total_kap. '&picket_width=' . $cutting_sheet_article->getPicketWidth() ?>">
