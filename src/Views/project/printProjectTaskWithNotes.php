...
$html = '
  <style>table {padding: 3px 10px 3px 10px; }</style>
  ...
  <table border="0">
    ...
    <tr>
      <td>adresa:</td>
      <td>' . $client['street'] . ' ' . $client['home_number'] . ', ' . $client['city'] . ', ' . $client['country'] . ', ' . $client['address_note'] . '</td>
    </tr>
    <tr>
      <td></td>
...
foreach ($notes as $note):
  ...
  <table>
    <tr>
      <td width="90px">' . ($note->getCreatedAt()->format('d-M-Y') != $date_temp ? $note->getCreatedAt()->format('d-M-Y') : "") . '</td>
      <td width="570px">' . nl2br($note->getNote()) . '</td>
    </tr>
  </table>
  ';
  $pdf->writeHTML($html, true, false, true, false, '');
