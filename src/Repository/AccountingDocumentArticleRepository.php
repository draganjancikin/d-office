...
class AccountingDocumentArticleRepository extends EntityRepository
{
    ...
    public function getQuantity($ad_article_id, $min_calc_measure, $pieces) {
        $properties = $this->getEntityManager()
          ...
          ->findBy(array('accounting_document_article' => $ad_article_id), []);
    ...
    public function duplicateArticleInAccountingDocument($accounting_document___article__id){
        $accounting_document__article = $this->getEntityManager()->find("\App\Entity\AccountingDocumentArticle", $accounting_document___article__id);

        $article = $this->getEntityManager()->find("\App\Entity\Article", $accounting_document__article->getArticle()->getId());
        ...
        $this->getEntityManager()->persist($newAccountingDocumentArticle);
        $this->getEntityManager()->flush();


        // Insert Article properties in table v6__accounting_documents__articles__properties.
        $article_properties = $this->getEntityManager()->getRepository('\App\Entity\ArticleProperty')->getArticleProperties($article->getId());
        ...
        foreach ($article_properties as $article_property) {
          // Insert to table v6__accounting_documents__articles__properties.
            ...
            $this->getEntityManager()->persist($newAccountingDocumentArticleProperty);
            $this->getEntityManager()->flush();
        }
