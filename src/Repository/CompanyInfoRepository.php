...
class CompanyInfoRepository extends EntityRepository
{
    ...
    public function getCompanyInfoData($id): array {
        $company_info_data = $this->getEntityManager()->find('\App\Entity\CompanyInfo', $id);
        if ($company_info_data->getCountry() === null) {
            $company_info_country = null;
        }
        else {
            $company_info_country = $this->getEntityManager()->find('\App\Entity\Country', $company_info_data->getCountry());
        }
        if ($company_info_data->getCity() === null) {
            $company_info_city = null;
        }
        else {
            $company_info_city = $this->getEntityManager()->find('\App\Entity\City', $company_info_data->getCity());
        }
        if ($company_info_data->getStreet() === null) {
            $company_info_street = null;
        }
        else {
            $company_info_street = $this->getEntityManager()->find('\App\Entity\Street', $company_info_data->getStreet());
        }
